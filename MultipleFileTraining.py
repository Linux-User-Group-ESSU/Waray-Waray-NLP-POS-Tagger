...
def train_hmm(dataset, trainer, train_number, total_train):
    ...
    # KFold for unbiased training and testing
    ...
    for train, test in tqdm(fold.split(dataset), total=split, desc=f"Training {train_number}/{total_train}..."):
        ...
        # Train the HMM model using WittenBellProbDist smoothing
        tagger = trainer.train_supervised(
            train_data, estimator=lambda fd, bins: WittenBellProbDist(fd, bins)
        )
...
def main():
    ...
    trainer = hmm.HiddenMarkovModelTrainer(states=STATES)
